...
function dnsRecordsHtml(dns){const records=dns.records||{};const types=Object.keys(records);let html='<div class="card" style="margin-top:20px"><h3>DNS Records</h3>';html+=`<p><strong>Provider:</strong> ${esc(dns.dns_provider||'Unknown provider')}<br><strong>Nameservers:</strong> ${esc((dns.dns_nameservers||[]).join(', ')||'N/A')}<br><strong>Record types checked:</strong> ${esc((dns.record_types_checked||types).join(', '))}<br><strong>DNSSEC:</strong> ${dns.dnssec?'Detected':'Not detected'}<br><strong>SPF:</strong> ${esc(dns.spf||'MISSING')}<br><strong>DMARC:</strong> ${esc(dns.dmarc||'MISSING')}</p>`;html+='<div style="overflow:auto"><table><thead><tr><th>Type</th><th>Record</th></tr></thead><tbody>';types.forEach(type=>{const rows=records[type]||[];if(!rows.length)return;rows.forEach(row=>{html+=`<tr><td><strong>${esc(type)}</strong></td><td style="white-space:pre-wrap;word-break:break-word">${esc(formatRecordValue(row,type))}</td></tr>`})});html+='</tbody></table></div></div>';return html;}
