...
<div class="card form">
    <form method="post" action="{{ $service->exists ? route('admin.services.update',$service) : route('admin.services.store') }}">
        ...
        <div class="field"><label>Billing Cycle</label><select class="input" id="cost_billing_cycle" name="cost_billing_cycle"><option value="monthly" @selected(old('cost_billing_cycle',$service->cost_billing_cycle ?: 'monthly')==='monthly')>Monthly</option><option value="quarterly" @selected(old('cost_billing_cycle',$service->cost_billing_cycle)==='quarterly')>Quarterly</option><option value="yearly" @selected(old('cost_billing_cycle',$service->cost_billing_cycle)==='yearly')>Yearly</option><option value="one_time" @selected(old('cost_billing_cycle',$service->cost_billing_cycle)==='one_time')>One-time</option></select></div>
        ...
        <div id="ftth-payment-fields" style="display:none;border:1px solid #e5e7eb;border-radius:8px;padding:12px;margin:10px 0 14px">
            ...
            <div class="field"><label>Payment Due Day *</label><input class="input" type="number" min="1" max="31" name="payment_due_day" value="{{ old('payment_due_day',$service->payment_due_day ?: 15) }}" placeholder="15"><small class="muted">Chỉ nhập ngày trong tháng. Không cần nhập tháng/năm. Ví dụ 15 = hàng tháng thanh toán trước/ngày 15.</small></div>
