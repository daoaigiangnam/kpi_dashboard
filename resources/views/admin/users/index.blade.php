...
<div class="card">
 <div style="display:flex;justify-content:space-between;gap:12px;align-items:center;flex-wrap:wrap;margin-bottom:15px">
  <div><strong>User Management</strong><div class="muted">Employee profile, integration identity, organization, access group and KPI job title are managed here.</div></div>
  <div class="actions">
 ...
 <form method="get" action="{{ route('admin.users.index') }}" style="display:flex;gap:8px;margin-bottom:15px;max-width:1100px;align-items:center;flex-wrap:wrap">
  <input class="input" name="search" value="{{ $search }}" placeholder="Search by employee code, external ID, name, email, phone, department or unit" style="margin-top:0;flex:1;min-width:280px">
 ...
 <div class="table-wrap"><table class="table" style="min-width:1450px"><thead><tr><th>Employee Code</th><th>External ID</th><th>Name</th><th>Email</th><th>Department</th><th>Unit</th><th>Job Title</th><th>Target WP</th><th>Group</th><th>Status</th><th>Actions</th></tr></thead><tbody>
 @forelse($users as $u)
 <tr>
  <td><strong>{{ $u->employee_code }}</strong></td><td>{{ $u->external_id ?? '—' }}</td><td>{{ $u->name }}</td><td>{{ $u->email }}</td><td>{{ $u->departmentRelation?->name ?? '—' }}</td><td>{{ $u->unit?->name ?? '—' }}</td><td>{{ $u->jobTitle?->name ?? '—' }}</td><td>{{ $u->jobTitle ? number_format((float)$u->jobTitle->target_workload_point,2).' WP' : '—' }}</td><td>{{ $u->group?->name ?? '—' }}</td>
 ...
 @empty
 <tr><td colspan="11" class="muted">{{ $showDeleted ? 'No deleted users found.' : 'No users found.' }}</td></tr>
 @endforelse
