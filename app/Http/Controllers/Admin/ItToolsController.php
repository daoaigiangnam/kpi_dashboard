...
class ItToolsController extends Controller
{
    ...
    public function audit(Request $request, InternetAssetAuditService $audit)
    {
        $data = $request->validate([
            'domain' => ['required','string','max:253'], 'wan_ip' => ['nullable','ip'],
            'dkim_selectors' => ['nullable','string','max:500'], 'service_hosts' => ['nullable','string','max:1000'],
        ]);
        $selectors = collect(preg_split('/[,\s]+/', (string) ($data['dkim_selectors'] ?? ''), -1, PREG_SPLIT_NO_EMPTY))
            ->map(fn ($value) => preg_replace('/[^a-z0-9._-]/i', '', $value))->filter()->unique()->take(20)->values()->all();
        $serviceHosts = collect(preg_split('/[,\s]+/', (string) ($data['service_hosts'] ?? ''), -1, PREG_SPLIT_NO_EMPTY))
            ->map(fn ($value) => preg_replace('/[^a-z0-9.-]/i', '', $value))->filter()->unique()->take(50)->values()->all();
        ...
        try {
            ...
            ItToolAudit::create(['user_id'=>auth()->id(),'domain'=>$data['domain'],'wan_ip'=>$data['wan_ip']??null,'status'=>'error','duration_ms'=>(int)round((microtime(true)-$started)*1000),'error'=>$e->getMessage()]);
            throw $e;
        }
    }
    ...
    {
        $query=ItToolAudit::query()->latest(); if($request->filled('domain')) $query->where('domain','like','%'.trim($request->string('domain')).'%');
    ...
    {
        $domain=$request->input('domain'); $filename='it-tool-audits-'.now()->format('Ymd-His').'.xlsx';
        $directory=sys_get_temp_dir().DIRECTORY_SEPARATOR.'it-tools-exports'; $path=$directory.DIRECTORY_SEPARATOR.$filename;
        try {
            ...
            $limit=min(max((int)$request->input('limit',1000),1),1000); $excel->setExportLimit($limit);
            $writer=$excel->output($domain); $writer->setPreCalculateFormulas(false); $writer->save($path);
            ...
            if(is_file($path)) @unlink($path); Log::error('IT Tools Excel export failed',['domain'=>$domain,'limit'=>$request->input('limit',1000),'exception'=>get_class($e),'message'=>$e->getMessage(),'memory'=>memory_get_usage(true),'peak_memory'=>memory_get_peak_usage(true)]);
