...
{
    public function index(){ return view('admin.it-tools.workbench'); }
    ...
    public function audit(Request $request, InternetAssetAuditService $audit)
    {
        ...
        try{
            ...
            ItToolAudit::create(['user_id'=>auth()->id(),'domain'=>$data['domain'],'wan_ip'=>$data['wan_ip']??null,'status'=>'completed','duration_ms'=>(int)round((microtime(true)-$started)*1000),'result'=>$result]);
            return response()->json($result);
        }catch(\Throwable $e){
            ItToolAudit::create(['user_id'=>auth()->id(),'domain'=>$data['domain'],'wan_ip'=>$data['wan_ip']??null,'status'=>'error','duration_ms'=>(int)round((microtime(true)-$started)*1000),'error'=>$e->getMessage()]); throw $e;
        }
    }
    ...
    public function importBulk(Request $request, BulkAuditImportService $importer){$data=$request->validate(['file'=>['required','file','max:5120','mimes:xlsx,xls,csv,txt']]);return response()->json($importer->import($data['file']->getRealPath(),100));}

    public function template(): StreamedResponse
    {
        $spreadsheet=new Spreadsheet();$sheet=$spreadsheet->getActiveSheet();$sheet->fromArray([['Domain','WAN IP'],['example.com','1.2.3.4'],['example.vn','']]);$sheet->getColumnDimension('A')->setAutoSize(true);$sheet->getColumnDimension('B')->setAutoSize(true);
    ...
    {
        $domain=$request->input('domain');$filename='it-tool-audits-'.now()->format('Ymd-His').'.xlsx';$directory=sys_get_temp_dir().DIRECTORY_SEPARATOR.'it-tools-exports';$path=$directory.DIRECTORY_SEPARATOR.$filename;
        try{
            if(!is_dir($directory)&&!mkdir($directory,0775,true)&&!is_dir($directory))throw new \RuntimeException('Unable to create IT Tools temporary export directory.');if(!is_writable($directory))throw new \RuntimeException('IT Tools temporary export directory is not writable.');
            $limit=min(max((int)$request->input('limit',1000),1),1000);$excel->setExportLimit($limit);$writer=$excel->output($domain);$writer->setPreCalculateFormulas(false);$writer->save($path);if(!is_file($path)||filesize($path)<100)throw new \RuntimeException('Excel file was not generated correctly.');
        ...
        }catch(\Throwable $e){if(is_file($path))@unlink($path);Log::error('IT Tools Excel export failed',['domain'=>$domain,'limit'=>$request->input('limit',1000),'exception'=>get_class($e),'message'=>$e->getMessage(),'memory'=>memory_get_usage(true),'peak_memory'=>memory_get_peak_usage(true)]);abort(500,'IT Tools Excel export failed: '.$e->getMessage());}
    }
}
