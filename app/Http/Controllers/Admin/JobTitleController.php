...
class JobTitleController extends Controller
{
    public function index(Request $request)
    {
        ...
        $jobTitles = JobTitle::withCount('users')
    ...
    public function export(Request $request)
    {
        ...
        foreach ($jobTitles as $jobTitle) {
            $sheet->fromArray([[
                ...
                $jobTitle->name,
                $jobTitle->level,
                (float) $jobTitle->target_workload_point,
        ...
        $filename = 'job-titles-'.now()->format('Ymd-His').'.xlsx';
    ...
    public function import(Request $request)
    {
        ...
        try {
            ...
            return back()->withErrors('The Excel file could not be read. Please use the Job Title export/template format.');
        }

        if (count($rows) < 2) {
            return back()->withErrors('The import file contains no data rows.');
        }
        ...
        if ($missing) {
            return back()->withErrors('Invalid template. Missing columns: '.implode(', ', $missing).'.');
        }
        ...
        foreach (array_slice($rows, 1, null, true) as $rowNumber => $row) {
            ...
            $name = trim((string) ($row[$columns['job title']] ?? ''));
            $level = trim((string) ($row[$columns['level']] ?? ''));
            $description = trim((string) ($row[$columns['description']] ?? ''));
            $target = $row[$columns['target workload point']] ?? null;
            $status = strtolower(trim((string) ($row[$columns['status']] ?? 'active')));

            if ($code === '' && $name === '' && ($target === null || $target === '')) {
                continue;
            }

            if ($code === '' || !preg_match('/^[A-Za-z0-9_-]+$/', $code)) {
                $errors[] = "Row {$rowNumber}: Code is required and may contain only letters, numbers, hyphens and underscores.";
                continue;
            }
            if ($name === '') {
                $errors[] = "Row {$rowNumber}: Job Title is required.";
                continue;
            }
            if (!is_numeric($target) || (float) $target < 0 || (float) $target > 99999999.99) {
                $errors[] = "Row {$rowNumber}: Target Workload Point must be a number from 0 to 99,999,999.99.";
                continue;
            }
            ...
            if (isset($seenCodes[$code])) {
                $errors[] = "Row {$rowNumber}: Duplicate Code '{$code}' in the import file.";
                continue;
            }
            ...
            $prepared[] = [
                ...
                'target_workload_point' => round((float) $target, 2),
                'is_active' => $status === 'active',
            ];
        }

        if ($errors) {
            return back()->withErrors(array_slice($errors, 0, 50))->with('import_error_count', count($errors));
        }

        if (!$prepared) {
            return back()->withErrors('The import file contains no valid data rows.');
        }

        DB::transaction(function () use ($prepared) {
            foreach ($prepared as $data) {
                JobTitle::updateOrCreate(['code' => $data['code']], $data);
            }
        });
