...
class GroupController extends Controller {
 public function index(){return view('admin.groups.index',['groups'=>UserGroup::withCount('users')->with('permissions')->orderBy('name')->paginate(20)]);}
 ...
 public function destroy(UserGroup $group){if($group->is_system)return back()->withErrors('System groups cannot be deleted.');if($group->users()->exists())return back()->withErrors('Group still has users.');$group->delete();return back()->with('success','Group deleted.');}
}
