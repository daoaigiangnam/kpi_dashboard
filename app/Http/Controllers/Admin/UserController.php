...
class UserController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            return redirect()->route('admin.users.index')->with('success', 'User created as inactive. Activate the account before sending a password setup email.');
        } catch (\Throwable $e) {
            report($e);
    ...
    public function template()
    {
        ...
        $sheet->fromArray([
            ['Employee Code','External ID','Name','Email','Phone','Date of Birth','Gender','Join Date','Department','Unit','Group','Job Title Code','Notes','Status'],
            ['EMP-0001','BITRIX-USER-1001','Example Employee','employee@example.com','','1990-01-15','Male','2026-01-01','IT','HCMC','Employee','IT-HELPDESK-L1','Example only - remove this row before import.','Active'],
        ], null, 'A1');
    ...
    public function export(Request $request)
    {
        ...
        $sheet->fromArray([['Employee Code','External ID','Name','Email','Phone','Date of Birth','Gender','Join Date','Department','Unit','Group','Job Title Code','Job Title','Target Workload Point','Notes','Status']],null,'A1');
        $row=2; foreach($users as $u){$status=$u->trashed()?'Deleted':($u->is_active?'Active':'Inactive');$sheet->fromArray([[$u->employee_code,$u->external_id,$u->name,$u->email,$u->phone,$u->date_of_birth?->format('Y-m-d'),$u->gender,$u->join_date?->format('Y-m-d'),$u->departmentRelation?->name,$u->unit?->name,$u->group?->name,$u->jobTitle?->code,$u->jobTitle?->name,$u->jobTitle?->target_workload_point,$u->notes,$status]],null,"A{$row}");$row++;}
        $sheet->getStyle('A1:P1')->getFont()->setBold(true); foreach(range('A','P') as $column)$sheet->getColumnDimension($column)->setAutoSize(true); $sheet->freezePane('A2'); $sheet->setAutoFilter('A1:P'.max(1,$row-1));
    ...
    public function import(Request $request)
    {
        ...
        $required=['employee code','external id','name','email','phone','date of birth','gender','join date','department','unit','group','job title code','notes','status'];
        ...
        $columns=array_flip($headers); $errors=[]; $prepared=[]; $seen=[]; $seenExternal=[];
        foreach(array_slice($rows,1,null,true) as $n=>$row){
            $v=fn($h)=>trim((string)($row[$columns[$h]]??'')); $code=$v('employee code'); $externalId=$v('external id'); $name=$v('name'); $email=$v('email'); $status=strtolower($v('status')?:'active'); $groupName=$v('group'); $jobCode=$v('job title code'); $departmentName=$v('department'); $unitName=$v('unit');
            if($code===''&&$externalId===''&&$name===''&&$email==='')continue;
            ...
            if($externalId===''||mb_strlen($externalId)>100){$errors[]="Row {$n}: External ID is required and must be at most 100 characters.";continue;}
            ...
            if(isset($seenExternal[$externalId])){$errors[]="Row {$n}: Duplicate External ID '{$externalId}'.";continue;} $seenExternal[$externalId]=true;
            ...
            $existing=User::withTrashed()->where('employee_code',$code)->first() ?: User::withTrashed()->where('external_id',$externalId)->first() ?: User::withTrashed()->where('email',$email)->first();
            $data=['employee_code'=>$code,'external_id'=>$externalId,'name'=>$name,'email'=>$email,'phone'=>$v('phone')?:null,'date_of_birth'=>$dob?:null,'gender'=>$v('gender')?:null,'join_date'=>$join?:null,'department'=>$departmentName?:null,'location'=>$unitName?:null,'department_id'=>$department?->id,'unit_id'=>$unit?->id,'user_group_id'=>$group?->id,'job_title_id'=>$job?->id,'notes'=>$v('notes')?:null,'is_active'=>$status==='active','registration_status'=>'approved'];
        ...
        $mailFailures=0;
        foreach($newUsers as $newUser){if(!$newUser->is_active)continue;try{$this->sendPasswordSetupEmail($newUser, true);}catch(\Throwable $e){report($e);$mailFailures++;}}
        $message=count($prepared).' user(s) imported successfully.';
        if(count($newUsers)>0)$message.=' New active user accounts receive a password setup email with OTP and secure link.';
    ...
    private function rules(?User $user=null, bool $creating=true): array
    {
        return [
            ...
            'external_id'=>['required','string','max:100',Rule::unique('users','external_id')->ignore($user?->id)],
