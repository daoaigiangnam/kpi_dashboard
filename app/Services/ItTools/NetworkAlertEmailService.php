...
class NetworkAlertEmailService
{
    public function process(): int
    {
        ...
        $sent = 0;
        ServiceMonitorEvent::query()->with(['service.customer', 'service.responsibleIt', 'service.provider', 'service.serviceType'])
            ->where(function ($q) { $q->where('status', 'open')->orWhere('event_type', 'recovered'); })
    ...
    private function sendLevel(ServiceMonitorEvent $event, int $level, string $emailType): bool
    {
        ...
        $alreadySent = ServiceMonitorEmailLog::query()->where('service_monitor_event_id', $event->id)->where('level', $level)->where('recipient_email', $recipient['email'])->where('email_type', $emailType)->where('status', 'sent')->exists();
        if ($alreadySent) return false;
        try {
            Mail::html($this->renderHtml($event, $emailType), function ($message) use ($recipient, $event, $emailType) {
                $isWebsite = strtoupper((string) $event->service?->serviceType?->code) === 'WEBSITE';
                $prefix = $emailType === 'resolution' ? '[RESOLVED]' : ($isWebsite ? '[WEBSITE DOWN]' : '[FTTH DOWN]');
                $message->to($recipient['email'], $recipient['name'])->subject($prefix.' IT Monitoring - '.$event->service->service_name);
            });
            ServiceMonitorEmailLog::updateOrCreate(['service_monitor_event_id' => $event->id, 'level' => $level, 'recipient_email' => $recipient['email'], 'email_type' => $emailType], ['recipient_type' => $recipient['type'], 'sent_at' => now(), 'status' => 'sent', 'error' => null]);
            return true;
        } catch (Throwable $e) {
            ServiceMonitorEmailLog::updateOrCreate(['service_monitor_event_id' => $event->id, 'level' => $level, 'recipient_email' => $recipient['email'], 'email_type' => $emailType], ['recipient_type' => $recipient['type'], 'sent_at' => null, 'status' => 'failed', 'error' => mb_substr($e->getMessage(), 0, 2000)]);
            return false;
        }
    }
    ...
    {
        $service = $event->service; $customer = $service?->customer;
        if ($level === 1) {
            $user = $service?->responsibleIt;
            return $user?->email ? ['type' => 'responsible_it', 'email' => $user->email, 'name' => $user->name ?: 'Operations Staff'] : null;
        }
        ...
        return $customer?->email ? ['type' => 'customer', 'email' => $customer->email, 'name' => $customer->contact_name ?: $customer->name] : null;
    }
    ...
    {
        $service = $event->service; $customer = $service?->customer; $isResolved = $emailType === 'resolution';
        $isWebsite = strtoupper((string) $service?->serviceType?->code) === 'WEBSITE';
        $status = $isResolved ? 'RESOLVED' : 'OFFLINE';
        $title = $isResolved ? ($isWebsite ? 'Website đã hoạt động trở lại' : 'Đường truyền FTTH đã khôi phục') : ($isWebsite ? 'CẢNH BÁO WEBSITE OFFLINE' : 'CẢNH BÁO FTTH / INTERNET OFFLINE');
        $message = $isResolved ? 'Hệ thống xác nhận dịch vụ đã hoạt động trở lại.' : ($isWebsite ? 'Hệ thống phát hiện website không thể kết nối tới các TCP port đã cấu hình.' : 'Hệ thống phát hiện đường truyền Internet/FTTH không thể kết nối theo phương thức giám sát đã cấu hình.');
        $started = optional($event->started_at)->format('d/m/Y H:i:s') ?: '-'; $resolved = optional($event->resolved_at)->format('d/m/Y H:i:s') ?: '-';
        $duration = $event->duration_seconds !== null ? gmdate('H:i:s', max(0, (int) $event->duration_seconds)) : '-';
        $latency = $event->latency_ms !== null ? $event->latency_ms.' ms' : '-'; $loss = $event->packet_loss_percent !== null ? $event->packet_loss_percent.'%' : '-';
        return '<!doctype html><html><body style="margin:0;background:#f3f4f6;font-family:Arial,Helvetica,sans-serif;color:#172033">'
            ...
            .'<tr><td style="border-bottom:1px solid #e5e7eb;color:#64748b">Port</td><td style="border-bottom:1px solid #e5e7eb;font-weight:700">'.e($event->port ?: '-').'</td></tr>'
